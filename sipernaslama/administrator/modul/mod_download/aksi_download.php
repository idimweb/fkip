...
elseif ($module=='download' AND $act=='update'){
  ...
  if (empty($lokasi_file)){
    mysqli_query($koneksi, "UPDATE download SET judul = '$_POST[judul]' WHERE id_download = '$_POST[id]'");
  }else{
    UploadFile($nama_file);
    mysqli_query($koneksi, "UPDATE download SET judul = '$_POST[judul]', nama_file    = '$nama_file' WHERE id_download = '$_POST[id]'");
  }
  header('location:../../media.php?module='.$module);
}
}
