...
switch($_GET['act']){
   ...
   echo "<div class='col-xs-12'>  
              <div class='box'>
                <div class='box-header'>
                  <h3 class='box-title'>Semua Komentar Video</h3>
                </div>
                <div class='box-body'>
                  <table id='example1' class='table table-bordered table-striped'>
                    ...
                    <tbody>";
                    $tampil=mysqli_query($koneksi, "SELECT * FROM komentarvid a JOIN video b ON a.id_video=b.id_video ORDER BY a.id_komentar DESC");
                    $no = 1;
                    while ($r=mysqli_fetch_array($tampil)){
                      ...
                      echo "<tr> 
                              <td>$no</td>
                              <td><a href='../play-$r[id_video]-$r[video_seo].html' target='blank'>$r[nama_komentar]</a></td>
                              <td>$isi,..</td>
                              <td><center>$r[aktif]</center></td>
                              <td><a class='btn btn-success btn-xs' title='Edit Data' href='?module=komentarvid&act=edit&id=$r[id_komentar]'><span class='glyphicon glyphicon-edit'></span></a>
                                  <a class='btn btn-danger btn-xs' title='Delete Data' href='$aksi?module=komentarvid&act=hapus&id=$r[id_komentar]' onclick=\"return confirm('Apa anda yakin untuk hapus Data ini?')\"><span class='glyphicon glyphicon-remove'></span></a>
                              </td>
                            </tr>";
                      $no++;
                    }
  ...
  case "edit":
  $r    = mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM komentarvid WHERE id_komentar='$_GET[id]'"));
  echo "<div class='col-md-12'>
              ...
              <div class='box-body'>
              <form class='form-horizontal' role='form' method=POST action='$aksi?module=komentarvid&act=update' enctype='multipart/form-data'>
