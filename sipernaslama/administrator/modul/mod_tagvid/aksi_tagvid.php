...
if ($module=='tagvid' AND $act=='hapus'){
  mysqli_query($koneksi, "DELETE FROM tagvid WHERE id_tag='$_GET[id]'");
...
elseif ($module=='tagvid' AND $act=='input'){
  ...
  mysqli_query($koneksi, "INSERT INTO tagvid(nama_tag,
                                             username,
                                             tag_seo) 
...
elseif ($module=='tagvid' AND $act=='update'){
  $tag_seo = seo_title($_POST['nama_tag']);
  mysqli_query($koneksi, "UPDATE tagvid SET nama_tag = '$_POST[nama_tag]', 
