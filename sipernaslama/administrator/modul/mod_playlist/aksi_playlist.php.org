...
if ($module=='playlist' AND $act=='input'){
  ...
  if (!empty($lokasi_file)){
    ...
    mysqli_query($koneksi, "INSERT INTO playlist( jdl_playlist,
    	                                  username,
                                        playlist_seo,
                                        gbr_playlist) 
...
elseif ($module=='playlist' AND $act=='update'){
  ...
  if (empty($lokasi_file)){
    mysqli_query($koneksi, "UPDATE playlist SET jdl_playlist = '$_POST[jdl_playlist]',
                                   playlist_seo     = '$playlist_seo', 
                                   aktif='$_POST[aktif]' WHERE id_playlist = '$_POST[id]'");
  }else{
    UploadPlaylist($nama_file_unik);
    $data_gambar = mysqli_query($koneksi, "SELECT gbr_playlist FROM playlist WHERE id_playlist='$_POST[id]'");
