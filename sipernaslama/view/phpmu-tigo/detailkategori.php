								...
								<div class="content-block main left">
									...
									<div class="block">
										<div class="block-title" style="background: #2182b4;">
											...
											<?php if (anti_injection($_GET[halkategori]) == ''){ ?>
												<h2>Berita Kategori "<?php echo "$n[nama_kategori]"; ?>"</h2>
											<?php }else{ ?>
												<h2>Berita Kategori "<?php echo "$n[nama_kategori] - Page. ".anti_injection($_GET[halkategori]); ?>"</h2>
											<?php } ?>
										...
										<div class="block-content">
											<?php
											  ...
											  $sql   = "SELECT * FROM berita 
															left join users on berita.username=users.username
																left join kategori on berita.id_kategori=kategori.id_kategori WHERE  berita.status='Y' AND berita.id_kategori='".anti_injection($_GET['kt'])."' 
											...
											$jmldata     = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM berita WHERE id_kategori='".anti_injection($_GET['kt'])."'"));
